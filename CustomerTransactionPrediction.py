...
# This function looks for errors in the dataset provided by user before applying the ML algorithmns on it.
...
def checkErrors(x_test):
    ...
    #2. Check if there are total of 201 columns ( i.e. ID_code and 200 features) 
    ...
    #3. Check if the column name 'ID_code' is present as the first column  
    ...
    #4. Check if all values of the dataframe are of type int or float
    error_cols =[]
...
# Function to perform any preprocessing steps
def preprocessing(df):
    # look for missing values in the dataset and replace them with the the median of the values in the same column
    column_medians = df.median()
    df = df.fillna(column_medians)
    return df
            
# Function that makes prediction on the input dataset
def predict(df):
    # my_bar is used for aesthetic purpose to show a progress bar during prediction
    ...
    # tranform the x_test to arrive at 209 features
    ...
    st.write(y_pred)
    y_pred_file =y_pred.to_csv(index=False).encode('utf-8')
    
    #Option to allow user to download the predictions as .csv file
...
# Loading the joblib files which has the trained classifiers

lgbm_multiclass = joblib.load("lgbm_multiclass.pkl")
lgbm_final = joblib.load("lgbm_final_binary.pkl")
